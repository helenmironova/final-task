...
const GridHeaders = (props: any) => {
    ...
    const handleLogoClick = (clicked: number) => {
        if(props.selected===clicked){
            if(type===1) props.setSelected(0); //if I clicked on a sorting icon that was sorting desc, selected points to 0 (nothing);
  ...
  return (
    <div className='gridHeaders'>
      ...
      <div className='entry'>
        ...
        <img src={props.selected===1 ? (type===0 ? logo2 : logo3) : logo1} className='logo' onClick={()=>handleLogoClick(1)} />
      </div>
      <div className='entryName'>
        <p className='entryP'>Entry Names</p>
        <img src={props.selected===2 ? (type===0 ? logo2 : logo3) : logo1} className='logo' onClick={()=>handleLogoClick(2)} />
      </div>
      <div className='entryName'>
        <p className='entryP'>Genes</p>
        <img src={props.selected===3 ? (type===0 ? logo2 : logo3) : logo1} className='logo' onClick={()=>handleLogoClick(3)} />
      </div>
      <div className='entryName'>
        <p className='entryP'>Organism</p>
        <img src={props.selected===4 ? (type===0 ? logo2 : logo3) : logo1} className='logo' onClick={()=>handleLogoClick(4)} />
      </div>
      ...
      <div className='length'>
        ...
        <img src={props.selected===5 ? (type===0 ? logo2 : logo3) : logo1} className='logo' onClick={()=>handleLogoClick(5)} />
      </div>
    </div>
  );
};
