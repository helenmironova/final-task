  ...
  return (
    <Fragment>
      <Routes>
        <Route
          path="/"
          element={authState.user ? <Navigate to="/search" /> : <MainPage />}
        />
        <Route
          path="/auth"
          element={authState.user ? <Navigate to="/search" /> : <LoginPage />}
        />
        <Route
          path="/auth/signup"
          element={authState.user ? <Navigate to="/search" /> : <SignupPage />}
        />
        <Route path="/not-found" element={<NotFoundPage />} />
        <Route path="*" element={<Navigate to="/not-found" />} />
        <Route path="/search" element={<SearchPage />} />
        <Route path={`/protein/${proteinDetails.id}`} element={<ProteinPage />}>
          <Route
            path={`/protein/${proteinDetails.id}/details`}
            element={<ProteinDetails />}
          />
        </Route>
      </Routes>
    </Fragment>
